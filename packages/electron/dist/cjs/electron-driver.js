...
class ElectronDriver {
    ...
    async startDevServer(projectPath, startScript) {
        return new Promise((resolve, reject) => {
            ...
            const devServerProcess = (0, child_process_1.spawn)(npmCmd, ['run', startScript], {
                cwd: projectPath,
                stdio: 'pipe',
                shell: true,
            });
    ...
    async getElectronInstance(projectPath) {
        ...
        try {
            ...
            await fs_1.promises.access(projectPlaywrightDir);
            ...
            try {
                ...
                projectPlaywright = await Promise.resolve(`${projectPlaywrightPath}`).then(s => __importStar(require(s)));
    ...
    async launchDevelopment(opts) {
        ...
        try {
            await fs_1.promises.stat(projectPath);
        }
        catch (error) {
        ...
        try {
            await fs_1.promises.stat(packageJsonPath);
            packageJsonContent = JSON.parse(await fs_1.promises.readFile(packageJsonPath, 'utf8'));
        }
        catch (error) {
        ...
        try {
            await fs_1.promises.stat(electronPath);
        }
        catch (error) {
    ...
    async isElectronForgeProject(projectPath, packageJsonContent) {
        ...
        for (const file of forgeFiles) {
            try {
                await fs_1.promises.access(path.join(projectPath, file));
                return true;
            }
            catch {
                // File doesn't exist
    ...
    async launchElectronForgeProject(opts, projectPath, packageJsonContent) {
        ...
        try {
            ...
            try {
                ...
                const playwrightModule = await Promise.resolve().then(() => __importStar(require('playwright-core')));
    ...
    async isPackagedApp(appPath) {
        try {
            const stats = await fs_1.promises.stat(appPath);
            ...
            if (stats.isDirectory()) {
                ...
                try {
                    await fs_1.promises.access(path.join(appPath, 'resources', 'app.asar'));
                    return true;
                }
                catch {
                    return false;
    ...
    async hasPackageJson(projectPath) {
        try {
            await fs_1.promises.access(path.join(projectPath, 'package.json'));
            return true;
        }
        catch {
            return false;
    ...
    async findElectronExecutable(projectPath) {
        ...
        try {
            await fs_1.promises.access(relativeElectron);
        ...
        try {
            await fs_1.promises.access(localElectron);
    ...
    enhanceSnapshotWithRefs(snapshot) {
        if (!snapshot)
            return snapshot;
        let refCounter = 1;
        const addRefs = (node) => {
...
exports.ElectronDriver = ElectronDriver;
